...
# DO2 — full installer (downloaded by install.sh so mirrors/USB don’t serve stale copies).
# Disk menu: NUMBERS ONLY — 0, 1, 2, …
INSTALL_SCRIPT_REV="2026-02-10.1"
...
# ── Disk list: choose by index (0, 1, 2, …) — never type nvme0n1/sda here ──
echo ""
echo "------------------------------------------------------------------------"
echo "  >>> DISK: type ONLY the number 0 or 1 (or 2, …). NEVER type nvme0n1 or sda. <<<"
echo "  >>> DISQUE : tapez SEULEMENT le chiffre 0 ou 1 (ou 2, …). PAS le nom du disque. <<<"
echo "------------------------------------------------------------------------"
echo "Detected drives (ISO disk excluded when possible) / Disques détectés :"
echo ""
...
echo ""
echo "Enter ONLY the number in [brackets] / Entrez SEULEMENT le numéro entre crochets :"
exec < /dev/tty
read -rp "Disk number (0, 1, …) / Numéro du disque : " CHOICE

if ! [[ "$CHOICE" =~ ^[0-9]+$ ]]; then
  echo "ERROR / ERREUR : use a single digit like 0 or 1 — not the device name."
  exit 1
fi
if [[ "$CHOICE" -lt 0 ]] || [[ "$CHOICE" -ge ${#DISK_NAMES[@]} ]]; then
  echo "ERROR / ERREUR : out of range. Valid: 0 … $((${#DISK_NAMES[@]} - 1))."
  exit 1
fi
...
echo ""
echo "  Selected / Disque sélectionné : $DEV"
echo ""
